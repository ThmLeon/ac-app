...
{:else}
	<Card class="flex flex-col md:flex-row gap-4">
		...
		<div class="flex flex-1 flex-col justify-between">
			<CardHeader>
				<CardTitle>{data.data?.titel}</CardTitle>
				<!-- Increased font size -->
				<CardDescription class="text-sm text-gray-500">
					<strong>Beginn:</strong>
					{formatDate(data.data?.start_datum_zeit)}<br />
					<strong>Ende:</strong>
					{formatDate(data.data?.ende_datum_zeit)}<br />
					<strong>Bewerbungsfrist:</strong>
					{formatApplicationDeadline(data.data?.bewerbungs_deadline)}
				</CardDescription>
			</CardHeader>
			<CardContent class="flex gap-2">
				<Badge variant="default">Offen</Badge>
				<Badge variant="default">
					{data.data?.event_master.master_name.charAt(0).toUpperCase() +
						data.data?.event_master.master_name.slice(1)}
				</Badge>
			</CardContent>
	...
	<div class="flex gap-4 w-full">
		...
		<Card class="flex-1">
			...
			<CardContent>
				<div class="flex flex-wrap gap-4 w-full">
					{#each data.data?.event_verantwortliche! as verantwortlicher}
		...
		<Card class="w-[350px] flex-shrink-0">
			...
			<CardContent>
				{data.data?.ort_strasse_hausnummer}<br />
				{data.data?.ort_plz_stadt}
			</CardContent>
		</Card>
	</div>
{/if}
